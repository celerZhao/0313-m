      ...
      <cube-scroll ref="scroll" :options="options" :listenScroll="true">
        <div ref="scrollMain">
          ...
          <ul class="row form form-li">
            ...
            <li class="pt-5 pb-5 pl-15 pr-15 fs-10 c-gray">折扣优惠：<span class="c-red" id="discount_amount">-{{discount_amount}}</span>元，购买油量：<span class="c-red" id="litre_num">{{litre_num}}</span>升，每月可兑付：<span class="c-red" id="cash_litre_month">{{litre_num_month}}</span>升，到期后一次性兑付卡内剩余油量至账户余额。</li>
          </ul>
          ...
          <ul class="row form form-li" v-if="has_oilTank">
            <li v-if="goil_amount_use != '0.00'">
               ...
               */-->
              <label>获赠油箱抵扣 <span class="fs-10" ><span class="icon-tanhao c-gray fs-14"></span> 当前订单可用<span class="c-red">{{goil_litre_use}}升</span>，抵<span class="c-red">{{goil_amount_use}}元</span></span></label>
...
<script>
  ...
  export default {
    ...
    return {
      ...
      discount_amount:0,  // 折扣优惠金额
      litre_num:0,        // 购买总油量
      litre_num_month:0,   // 每月兑付油量

      discountrate:0,     // 加油卡折扣率
  ...
  ...mapActions([
    ...
    // 更新油量
    refreshOilAmount(){
      this.discount_amount = parseFloat(this.totalAmount*(1-parseFloat(this.discountrate)/10)).toFixed(2);
      this.litre_num = parseFloat(this.totalAmount/this.referenceOilPrice).toFixed(2);
      this.litre_num_month = (parseFloat(this.litre_num)/this.term).toFixed(2);
    },
    ...
    getData(_vmCard){
      ...
      api.productDfa({
        ...
        this.type_bn = res.type_bn;

        this.totalAmount = this.reco_cash_min;
        this.discountrate = res.sale_rule.discountrate; // 折扣率
        if(_vmCard != null){
...
//              _coupon  = null;
        ...
        this.payMoney = parseFloat(this.totalAmount-this.cashQuan-this.discount_amount);
        this.saveMoney = parseFloat(this.cashQuan + parseFloat(this.discount_amount));
...
//        if(payLeft>=rule_order_money){
      ...
      this.coupons = _couponArr;
      this.payMoney = parseFloat(this.totalAmount) - allCpnsUsed - parseFloat(this.discount_amount);
      this.pay_leftByRule = payLeft;      // 现金券抵扣规则剩余金额
      this.couponTipc = "- ¥ "+allCpnsUsed+" ";
      this.saveMoney = allCpnsUsed + parseFloat(this.discount_amount);
